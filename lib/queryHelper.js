...
module.exports.getAllData = async(model,query={},selectedField={}) => {
    return await  model.find(query,selectedField)
}

module.exports.getData = async(model,query={},selectedField={}) => {
    let data = await model.findOne(query,selectedField)
    if(data){
        return data
    }
    return false
